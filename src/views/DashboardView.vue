...
import { CalendarClock, ChevronRight, Plus, TimerReset } from '@lucide/vue'
import { useWorkshopStore } from '@/stores/workshop'
import type { WorkOrder } from '@/types/workshop'
import { formatCurrency, formatDate, initials } from '@/utils/format'
import BayBoard from '@/components/BayBoard.vue'
...
<template>
  <div class="page dashboard-page">
    ...
    <section class="metrics-grid">
      <MetricCard label="Cars in the shop" :value="store.activeOrders.length" :detail="`${store.dueToday.length} due today`" />
      <MetricCard label="Pipeline value" :value="formatCurrency(store.revenuePipeline)" :detail="`across ${store.orders.length} open orders`" />
      <MetricCard label="Avg. ticket" :value="formatCurrency(store.averageTicket)" detail="per work order" />
      <MetricCard label="Ready for pickup" :value="store.completedCount" detail="waiting on the customer" />
    </section>

    <BayBoard @select="selected = $event" />

    <section class="dashboard-grid">
      <div class="panel orders-panel">
        <header class="panel-header"><div><h3>Latest work orders</h3><p>The five most recent jobs on the board</p></div><RouterLink to="/work-orders" class="text-link">View all <ChevronRight :size="16" /></RouterLink></header>
      ...
      <aside class="panel schedule-panel">
        <header class="panel-header"><div><h3>Today’s schedule</h3><p>{{ schedule.length }} cars promised back today</p></div><span class="calendar-icon"><CalendarClock :size="19" /></span></header>
    ...
    <section class="panel capacity-panel">
      <header class="panel-header"><div><h3>Team capacity</h3><p>How much work each technician is carrying</p></div><span class="capacity-note"><i /> Balanced workload</span></header>
